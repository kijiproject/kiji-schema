...
/**
 * Tests for the {@link org.kiji.schema.KijiRowData} API. Implementations of {@code KijiRowData} should provide a
 * concrete implementation for this test suite.
 */
public class TestAsyncHBaseKijiRowData extends KijiClientTest {
  ...
  private Kiji mHBaseKiji;
  private Kiji mAsyncKiji;
  ...
  public final void setupKijiRowDataTest() throws Exception {
    mHBaseKiji = getKiji();
    mAsyncKiji = new AsyncHBaseKijiFactory().open(mHBaseKiji.getURI());
    mHBaseKiji.createTable(KijiTableLayouts.getLayout(TEST_LAYOUT_V1));
    mTable = mAsyncKiji.openTable(TABLE_NAME);
  ...
  /**
   * Create a {@link org.kiji.schema.KijiRowData} for the given table, reader, entity id, and data request.
   ...
   * @throws java.io.IOException On error creating the Kiji row data.
   */
  ...
  public void testEntityId() throws IOException {
    ...
    final KijiRowData data = getRowData(mReader, eid, KijiDataRequest.empty());
  ...
  public void testReadInts() throws IOException {
    ...
    final KijiRowData data = getRowData(mReader, eid, KijiDataRequest.create(FAMILY, QUALIFIER_3));
  ...
  public void testGetReaderSchemaLayout13() throws Exception {
    mHBaseKiji.createTable(KijiTableLayouts.getLayout(TEST_LAYOUT_V1_3));
    final KijiTable table = mAsyncKiji.openTable("table");
    try {
  ...
  public void testReadWithMaxVersions() throws IOException {
    ...
    final KijiRowData data = getRowData(mReader, eid, dataRequest);
  ...
  public void testContainsColumn() throws Exception {
    ...
    final KijiRowData data = getRowData(mReader, eid, KijiDataRequest.create(FAMILY, QUALIFIER_0));
  ...
  public void testMapAsIterable() throws IOException {
    ...
    final KijiRowData row1 = getRowData(mReader, eid, dataRequest);
  ...
  public void testReadDeletedColumns() throws Exception {
    ...
    mHBaseKiji.modifyTableLayout(update);
  ...
  public void testWriterSchemaWhenSpecificRecordClassNotFound() throws Exception {
    mHBaseKiji.createTable(KijiTableLayouts.getLayout(WRITER_SCHEMA_TEST));
    final KijiTable table = mAsyncKiji.openTable("writer_schema");
    try {
