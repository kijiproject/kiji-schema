...
/** Tests for AsyncHBaseKiji. */
public class TestAsyncHBaseKiji extends KijiClientTest {
  private static final Logger LOG = LoggerFactory.getLogger(TestAsyncHBaseKiji.class);
  ...
  public void testOpenUnknownTable() throws Exception {
    final Kiji kiji = new AsyncHBaseKiji(createTestURI());
  ...
  public void testModifyTableLayoutAddRemove() throws IOException {
    ...
    final Kiji hbaseKiji =  new InstanceBuilder(getKiji())
        ...
        .build();
    final Kiji kiji = new AsyncHBaseKiji(hbaseKiji.getURI());
    ...
    try {
      ...
      Assert.assertEquals(e.getMessage(), "AsyncHBaseKiji cannot add or remove columns");
    }
    ...
    try {
      ...
      Assert.assertEquals(e.getMessage(), "AsyncHBaseKiji cannot add or remove families");
    }
    ...
    try {
      ...
      Assert.assertEquals(e.getMessage(), "AsyncHBaseKiji cannot add or remove locality groups");
    }
