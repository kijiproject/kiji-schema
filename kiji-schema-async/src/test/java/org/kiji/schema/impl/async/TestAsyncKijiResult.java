...
public class TestAsyncKijiResult extends KijiClientTest {
  private static final Logger LOG = LoggerFactory.getLogger(TestAsyncKijiResult.class);
  ...
  // The reader is not used in tests, but it may not be closed until @After.
  private AsyncKijiTableReader mReader = null;
  private AsyncKiji mAsyncKiji = null;
  ...
  @Before
  public void setupTestAsyncKijiResult() throws IOException {
    Kiji kiji = getKiji();
            ...
            .withRow(1)
                ...
                .withFamily("string_map")
                    ...
                    .withQualifier("smap_2")
                        ...
                        .withValue(1, "sm2-one")
        .build();
    mAsyncKiji = new AsyncKiji(kiji.getURI());
    final AsyncKijiTable table = AsyncKijiTable.downcast(mAsyncKiji.openTable("all_types_table"));
    try {
      Log.info("Entered try");
      final EntityId eid = table.getEntityId(1);
      Log.info("Opening TableReader...");
      mReader = (AsyncKijiTableReader) table.openTableReader();
      Log.info("Getting Result for REQUEST from table");
      mResult = mReader.getResult(eid, REQUEST);
      Log.info("mResult: {}", mResult.toString());
      Log.info("Getting Result for PAGED_REQUEST from table");

      mPagedResult = mReader.getResult(eid, PAGED_REQUEST);
      Log.info("Getting Result for MAP_REQUEST from table");
      mMapResult = mReader.getResult(eid, MAP_REQUEST);
      Log.info("Getting Result for PAGED_MAP_REQUEST from table");
      mPagedMapResult = mReader.getResult(eid, PAGED_MAP_REQUEST);
      Log.info("Getting Result for COMPLETE_REQUEST from table");
      mCompleteResult = mReader.getResult(eid, COMPLETE_REQUEST);
      Log.info("Getting Result for PAGED_COMPLETE_REQUEST from table");
      mPagedCompleteResult = mReader.getResult(eid, PAGED_COMPLETE_REQUEST);
      Log.info("Getting Result for MAX_VERSIONS_REQUEST from table");
      mMaxVersionsResult = mReader.getResult(eid, MAX_VERSIONS_REQUEST);
      Log.info("Getting Result for PAGED_MAX_VERSIONS_REQUEST from table");
      mPagedMaxVersionsResult = mReader.getResult(eid, PAGED_MAX_VERSIONS_REQUEST);
      Log.info("Finished Try");
    } catch (Exception e) {
      Log.info("Uh oh there was an exception: {}", e.getStackTrace());
  ...
  public void testGets() throws IOException {
    ...
    }
    {
      try {
        ...
        Log.info("iae: {}", iae.getMessage());
        Log.info("expected: {}", EXPECTED_PAGED_GET_EXCEPTION_PREFIX);
