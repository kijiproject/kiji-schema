...
public class TestAsyncMetaTable extends KijiClientTest {
  private static final Logger LOG = LoggerFactory.getLogger(TestAsyncMetaTable.class);

  private KijiURI mKijiURI;
  private AsyncMetaTable mMetaTable;
  ...
  public final void setupTest() throws IOException {
    ...
    AsyncMetaTable.install(mAdmin, mKijiURI);
    mHBClient = new HBaseClient(mKijiURI.getZooKeeperEnsemble());
    mMetaTable = new AsyncMetaTable(
  ...
  public final void teardownTest() throws IOException {
    ...
    mMetaTable = null;
    mHBClient.shutdown();
    AsyncMetaTable.uninstall(mAdmin, mKijiURI);
  }
