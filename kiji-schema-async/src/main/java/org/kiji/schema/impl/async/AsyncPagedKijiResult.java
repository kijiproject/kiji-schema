 ...
 */
@ApiAudience.Private
public class AsyncPagedKijiResult<T> implements KijiResult<T> {
  ...
  /**
   * Create a new {@link org.kiji.schema.impl.async.AsyncPagedKijiResult}.
   *
  ...
  public final class PagedColumnIterable implements Iterable<KijiCell<T>>, Closeable {
    ...
    private void setUpScanner() {
      try {
        ...
        if (kijiFilter != null) {
          filter = AsyncDataRequestAdapter.toScanFilter(kijiFilter, mColumn, filterContext);
        } else {
          filter = null;
        }
        ...
        if (mColumn.isFullyQualified()) {
          AsyncDataRequestAdapter.addColumnOrFamily(
              familyMap,
          ...
          if (Arrays.equals(hbaseColumn.getQualifier(), new byte[0])) {
            ...
            AsyncDataRequestAdapter.addColumnOrFamily(familyMap, hbaseColumn.getFamily(), null);
            mScanner.setFilter(filter);
        ...
        AsyncDataRequestAdapter.setFamiliesForScanner(mScanner, familyMap);
