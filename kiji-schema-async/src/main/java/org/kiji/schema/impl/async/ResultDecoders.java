...
public final class ResultDecoders {
  ...
  /**
   * A function which will decode {@link org.apache.hadoop.hbase.KeyValue}s from a map-type column.
   *
   * <p>
  ...
  private static final class MapFamilyDecoder<T> implements Function<KeyValue, KijiCell<T>> {
    ...
    public KijiCell<T> apply(final KeyValue keyValue) {
      if (!Arrays.equals(mLastQualifier, keyValue.qualifier())) {
        mLastQualifier = keyValue.qualifier();
        try {
          mLastColumn =
              mColumnTranslator.toKijiColumnName(
                  new HBaseColumnName(keyValue.family(), keyValue.qualifier()));
      ...
      try {
        final DecodedCell<T> decodedCell = mCellDecoder.decodeCell(keyValue.value());
        return KijiCell.create(mLastColumn, keyValue.timestamp(), decodedCell);
      } catch (IOException e) {
  ...
  /**
   * A function which will decode {@link org.apache.hadoop.hbase.KeyValue}s from a group-type family.
   *
   * <p>
  ...
  private static final class GroupFamilyDecoder<T> implements Function<KeyValue, KijiCell<T>> {
    ...
    public KijiCell<T> apply(final KeyValue keyValue) {
      ...
      try {
        final DecodedCell<T> decodedCell = mLastDecoder.decodeCell(keyValue.value());
        return KijiCell.create(mLastColumn, keyValue.timestamp(), decodedCell);
      } catch (IOException e) {
        throw new KijiIOException(e);
      }
  ...
  /**
   * A function which will decode {@link org.apache.hadoop.hbase.KeyValue}s from a qualified column.
   *
   * <p>
  ...
  private static final class QualifiedColumnDecoder<T> implements Function<KeyValue, KijiCell<T>> {
    ...
    public KijiCell<T> apply(final KeyValue keyValue) {
      try {
        final DecodedCell<T> decodedCell = mCellDecoder.decodeCell(keyValue.value());
        return KijiCell.create(mColumnName, keyValue.timestamp(), decodedCell);
      } catch (IOException e) {
        throw new KijiIOException(e);
      }
    }
