...
import org.kiji.schema.layout.impl.AsyncHBaseTableLayoutDatabase;
 ...
 */
@ApiAudience.Private
public final class AsyncHBaseMetaTable implements KijiMetaTable {

  private static final Logger LOG = LoggerFactory.getLogger(AsyncHBaseMetaTable.class);
  private static final Logger CLEANUP_LOG =
      LoggerFactory.getLogger("cleanup" + AsyncHBaseMetaTable.class.getName());
  ...
  /** The HBaseClient of the Kiji instance this meta table belongs to. */
  ...
  //     so we can call AsyncHBaseMetaTable thread-safe, too.

  /**
   * Creates an HTableInterface for the specified table.
   *
   * @param kijiURI the KijiURI.
   * @param hbClient HBaseClient for the associated Kiji.
   * @return a new HTableInterface for the specified table.
   * @throws IOException on I/O error.
   */
public static byte[] newMetaTable(
    KijiURI kijiURI,
    HBaseClient hbClient)
    throws IOException {
    final byte[] tableName =
        KijiManagedHBaseTableName.getMetaTableName(kijiURI.getInstance()).toBytes();
    try {
...
/**
 * Create a connection to a Kiji meta table backed by an HBaseClient.
 *
...
AsyncHBaseMetaTable(
    ...
    throws IOException {
  this(kijiURI, hbClient, newMetaTable(kijiURI, hbClient), schemaTable);
}
 ...
 *
 * @param kijiURI URI of the Kiji instance this meta-table belongs to.
 * @param hbClient HBaseClient of the Kiji instance this meta-table belongs to.
 * @param tableName The name of the meta-table.
 * @param schemaTable The Kiji schema table.
 * @throws IOException If there is an error.
 */
private AsyncHBaseMetaTable(
    KijiURI kijiURI,
    HBaseClient hbClient,
    byte[] tableName,
    KijiSchemaTable schemaTable
  ...
  this(
      ...
      tableName,
      hbClient,
      new AsyncHBaseTableLayoutDatabase(kijiURI, hbClient, tableName, LAYOUT_COLUMN_FAMILY, schemaTable),
      new AsyncHBaseTableKeyValueDatabase(hbClient, tableName, META_COLUMN_FAMILY));
}
 ...
 * @param kijiURI URI of the Kiji instance this meta-table belongs to.
 * @param tableName The name of the meta-table.
 * @param hbClient The HBaseClient of the Kiji instance this meta-table belongs to.
 ...
 */
  ...
  public static void install(HBaseAdmin admin, KijiURI uri)
    ...
    tableDescriptor.addFamily(
      AsyncHBaseTableLayoutDatabase.getHColumnDescriptor(LAYOUT_COLUMN_FAMILY));
    tableDescriptor.addFamily(
      AsyncHBaseTableLayoutDatabase.getHColumnDescriptor(META_COLUMN_FAMILY));
  ...
  public void fromBackup(MetaTableBackup backup) throws IOException {
    ...
    LOG.info("Flushing commits to table '{}'", Bytes.pretty(mTableName));
  }
  ...
  public String toString() {
    return Objects.toStringHelper(AsyncHBaseMetaTable.class)
        .add("uri", mKijiURI)
