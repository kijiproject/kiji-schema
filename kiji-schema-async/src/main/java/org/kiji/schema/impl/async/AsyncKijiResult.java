 ...
 */
@ApiAudience.Private
public final class AsyncKijiResult<T> implements KijiResult<T> {

  private final KijiDataRequest mDataRequest;
  private final AsyncMaterializedKijiResult<T> mMaterializedResult;
  private final AsyncPagedKijiResult<T> mPagedResult;

  /**
   * Initialize a new {@link AsyncKijiResult}.
   *
   ...
   */
  private AsyncKijiResult(
      final KijiDataRequest dataRequest,
      final AsyncMaterializedKijiResult<T> materializedResult,
      final AsyncPagedKijiResult<T> pagedResult
  ) {
  ...
  /**
   * Create a new {@link AsyncKijiResult}.
   *
  ...
  public static <T> KijiResult<T> create(
    ...
    final AsyncMaterializedKijiResult<T> materializedKijiResult;
    if (!unpagedRequest.isEmpty()) {
      materializedKijiResult =
          AsyncMaterializedKijiResult.create(
              entityId,
              unpagedRequest,
              unpagedRawResult,
              layout,
    ...
    final AsyncPagedKijiResult<T> pagedKijiResult;
    if (!pagedRequest.isEmpty()) {
      pagedKijiResult =
          new AsyncPagedKijiResult<T>(
              entityId,
              pagedRequest,
              table,
              layout,
    ...
    if (unpagedRequest.isEmpty()) {
      ...
      return new AsyncKijiResult<T>(dataRequest, materializedKijiResult, pagedKijiResult);
    }
  }
