 ...
 * You can also use the {@link #columns()} method of this object to get instances of
...
public final class KijiDataRequestBuilder {
   ...
   *
   * <p>Use the {@link KijiDataRequestBuilder#columns()} method to get an instance of
  ...
  public final class ColumnsDef {
    ...
    public ColumnsDef withPageSize(int pageSize) {
      ...
      Preconditions.checkState(mPageSize == null,
          "Cannot set page size to %d, page size already set to %d.",
          pageSize, mPageSize);
      Preconditions.checkArgument(pageSize >= 0,
        "Page size must 0 (disabled) or positive, but got: " + mPageSize);
    ...
    public ColumnsDef addFamily(String family) {
      ...
      Preconditions.checkArgument(!family.contains(":"),
          ...
          "Family name cannot contain ':', but got '%s'.",
          family);
  ...
  public KijiDataRequestBuilder withTimeRange(long minTimestamp, long maxTimestamp) {
    ...
    Preconditions.checkArgument(minTimestamp >= 0,
      "minTimestamp must be positive or zero, but got: " + minTimestamp);
   ...
   *     data request builder.
   */
  public ColumnsDef columns(KijiDataRequest.Column existingColumn) {
    return columns()
  ...
  /**
   * Throws an exception if we have already used the {@link #build()} method, to
   * prevent reuse of this object.
   */
